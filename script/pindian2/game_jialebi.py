...
def GetConfig():
    ...
    amount = config.get("jialebi","amount")
    isdouble = config.get("jialebi","isdouble")
    #bet = base64.encodestring(bet)
    userData = {
...
#登录信息  
    strLoginInfo = {'accountType':'paw',
                ...
                'isapp':'1',
                'back_js':'',
                'back_url':'http://'+userData['testStr']+userData['environmentNum']+'-wap.stg2.24cp.com/?act=game_pindian2',
                'back_flag':'1',
                'register_step2':'',
                'tabs':'paw|wlt|toa',
                'accountType':'paw',
    ...
    isdouble = userData['isdouble']
    playInfo1 = {'gameRound':'1','amount':amount,'isdouble':isdouble,'custString':'custString','reqAppId':'1','reqTime':'11'}

    for i in range(num):
        jlburl = 'http://'+userData['testStr']+userData['environmentNum']+'-wap.stg2.24cp.com/?act=game_caribbeanpoker&st=play'
        result = PostUrlRequest(jlburl,playInfo1)
        result1 = str(i)+": "+result
        print(result1)
        jsonData = json.loads(result)
        batch_no = jsonData['body']['batch_no']
        playInfo2 = {'gameRound':'2','amount':amount,'isdouble':isdouble,'custString':'custString','batch_no':batch_no,'reqAppId':'1','reqTime':'11'}
        result2 = PostUrlRequest(jlburl,playInfo2)
        result2 = str(i)+": "+result2
        print(result2)
        #f = open("/data/python/script/logs/game_pindian2.log",mode='a',encoding="UTF-8")
        #f.write(result+"\n")
        #f.close()
