...
#登录信息  
    ...
    bet = str(userData['bet'])
    bet = base64.b64encode(str(bet).encode(encoding='utf-8'))
    playInfo = {'bet':bet}

    for i in range(num):
        pdurl = 'http://'+userData['testStr']+userData['environmentNum']+'-wap.stg2.24cp.com/?act=game_pindian2&st=play'
        result = PostUrlRequest(pdurl,playInfo)
        result = str(i)+": "+result
        print(result)
