...
def GetConfig():
    ...
    amount = config.get("niuniu","amount")
    userData = {
...
#登录信息  
    strLoginInfo = {'accountType':'paw',
                ...
                'isapp':'1',
                'back_js':'',
                'back_url':'http://'+userData['testStr']+userData['environmentNum']+'-wap.stg2.24cp.com/?act=game_niuniu',
                'back_flag':'1',
                'register_step2':'',
                'tabs':'paw|wlt|toa',
                'accountType':'paw',
    ...
    for i in range(num):
        betUrl = 'http://'+userData['testStr']+userData['environmentNum']+'-wap.stg2.24cp.com/?act=game_niuniu&st=play&amount='+userData['amount']
        result = str(i)+": "+GetUrlRequest(betUrl)
        print(result)
        f = open("/data/python/script/logs/game_niuniu.log",mode='a',encoding="UTF-8")
        f.write(result+"\n")
        f.close()
